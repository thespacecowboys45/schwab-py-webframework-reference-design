...
def task_template_asyncio_one_time():
    ...
    redis_client.set(saa.FLAG_ASYNCIO_ONE_TIME_TEMPLATE_TASK_IS_RUNNING, "true")
    status = {"status":"running"}
    redis_client.set(saa.FLAG_ASYNCIO_ONE_TIME_TEMPLATE_TASK_STATUS, str(status))

    while redis_client.get(saa.FLAG_ASYNCIO_ONE_TIME_TEMPLATE_TASK_IS_RUNNING) == b"true":
        task_iterations += 1
        ...
        redis_client.set(saa.FLAG_ASYNCIO_ONE_TIME_TEMPLATE_TASK_RESULT, str(result))  # Store the result in Redis   
...
def do_work():
    ...
    data = redis_client.get(saa.ASYNCIO_ONE_TIME_TEMPLATE_TASK_DATA)
...
def init():
    ...
    status = {"status": "init"}
    redis_client.set(saa.FLAG_ASYNCIO_ONE_TIME_TEMPLATE_TASK_STATUS, str(status))  # Store the result in Redis 

    result = {"result":"init"}
    redis_client.set(saa.FLAG_ASYNCIO_ONE_TIME_TEMPLATE_TASK_RESULT, str(result))  # Store the result in Redis 

    redis_client.set(saa.FLAG_ASYNCIO_ONE_TIME_TEMPLATE_TASK_IS_RUNNING, "false")
