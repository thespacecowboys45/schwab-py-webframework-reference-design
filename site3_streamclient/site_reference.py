...
# Tells the server to monitor the sourcecode for changes code (*.py, *.html, etc) and reload
...
# Operations to perform on every request
@app.before_request
def before_request():
    now = datetime.now()
    ...
    if request.path == '/' and 'session' in request.args:
        ...
        # We blank out the authorization_url so the navigation header works to show a 'logout' link
        g.authorization_url = ""
    else:
        ...
        if isLoggedInToSchwab() and request.path != '/logout':
            ...
            # Set authorization_url to blank so the header navigation works properly and shows a 'logout' link.
            g.authorization_url = ""
        else:
...
def home():
    ...
    if g.oauth_code:
        ...
        #       file on the server (or localhost) so that the
        #       token can be used in subsequent calls to schwab's API
        #
        try:
...
def printEnv():
    ...
    print(f"[printEnv] Environment Variable redirect_uri: {g.redirect_uri}")
...
#
# This function is used for application initialization
#
# Basic things like: resetting task flags
#
def appInit():
    # for dev/debug
    fn_name = inspect.currentframe().f_code.co_name
    ...
    # Reset celery task running flags
    from services.automations.automations_poller_task_template import init as ptt_init
    from services.automations.automations_asyncio_task_template import init as aio_init
    from services.stream_client import init as sc_init
